...
class Wpr_Sticky_Section {
    ...
    public function register_controls( $element ) {
		...
		if ( ( 'section' === $element->get_name() ) ) {
            // how to direct from edit template to specific page?
			...
			$element->add_control(
				...
				[
					'label' => esc_html__( 'Disable on Devices', 'wpr-addons' ),
					'label_block' => true,
					...
					// 'prefix_class' => 'wpr-sticky-disabled-on-',
					'multiple' => true,
					'separator' => 'before',
					'condition' => [
			...
			$element->add_responsive_control(
				...
				[
					...
					'mobile_default' => 0,
                    'selectors' => [
                        '{{WRAPPER}}' => '{{position_location.VALUE}}: {{VALUE}}px;',
                    ],
					'condition' => [
						'enable_sticky_section' => 'yes'
					],
	...
	public function responsive_breakpoints() {
		$breakpoints = Responsive::get_breakpoints();

		$available_devices = [];

		foreach ( $breakpoints as $key => $breakpoint ) {
			array_push( $available_devices, $key );
		}
