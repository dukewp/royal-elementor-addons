	...
	var WprElements = {
		...
		widgetSection: function( $scope ) {
			...
			function changeElementOffset() {
				// setOffsets = [offsetWideScreen, offsetDesktop, offsetLaptop, offsetTabletExtra, offsetTablet, offsetMobileExtra,  offsetMobile];
				...
				if ( 'inactive' === customBreakPoints || false === customBreakPoints ) {	
					...
					checkDevices.forEach((device, index) => {
						if ( (breakPointSizes[index] > viewportWidth) && availableDevices.indexOf(device) === -1 ) {
							...
							positionStyle = positionType;
							var adminBarHeight = $('#wpadminbar').length ? $('#wpadminbar').css('height').slice(0, $('#wpadminbar').css('height').length - 2) : 0;
