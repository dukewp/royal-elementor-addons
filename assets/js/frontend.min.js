		...
		widgetSection: function( $scope ) {
			...
			if( $scope.hasClass('wpr-sticky-section-yes') ) {
				var positionType = !WprElements.editorCheck() ? $scope.attr('data-wpr-position-type') : $scope.find('.wpr-sticky-section-yes-editor').attr('data-wpr-position-type'),
					...
					stickySectionClass = $scope.attr('class').split(),
				...
				if ( $('#wpadminbar').length && 'sticky' == $scope.css('position') ) {
					alert($('#wpadminbar').css('height').slice(0, $('#wpadminbar').css('height').length - 2));
			...
			function changePositionType(customBpoints) {
				if ( 'inactive' == customBpoints ) {						
					...
					if ( viewportWidth >= 2400 && availableDevices.indexOf('widescreen_sticky') == -1 ) {
						...
						positionStyle = 'relative';
					} else if ( (1201 <= viewportWidth && 1216 > viewportWidth) && availableDevices.indexOf('laptop_sticky') == -1 ) {
						positionStyle = 'relative';
