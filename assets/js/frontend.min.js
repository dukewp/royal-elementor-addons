	...
	var WprElements = {
		...
		init: function() {
			...
			var widgets = {
				...
				'wpr-advanced-table.default' : WprElements.widgetAdvancedTable,
		...
		widgetAdvancedTable: function($scope) {
			
			var beforeFilter = $scope.find("tbody .wpr-table-row"),
				pageIndex,
				itemsPerPage = +$scope.find('.wpr-table-inner-container').attr('data-rows-per-page'),
				paginationListItems = $scope.find('.wpr-table-container').find('.wpr-table-custom-pagination-list-item'),
				value, paginationIndex;

				var initialRows = $scope.find('.wpr-table-inner-container tbody tr');
				var table = $scope.find('.wpr-table-inner-container tbody');

			// Table Custom Pagination
			if ( 'yes' === $scope.find('.wpr-table-inner-container').attr('data-custom-pagination') ) {
				var tableRows = $scope.find('.wpr-table-inner-container tbody tr').filter(function(index) {
				...
				$(document).on('click', '.wpr-table-custom-pagination-list-item', function() {
				...
				$scope.find('.wpr-table-container').find('.wpr-table-prev-next').each(function() {
			...
			var tableColumns = $scope.find('.wpr-table-inner-container').attr('data-table-columns')
			if ($scope.find('.wpr-table-th').length > tableColumns) {

				var thisItemText = [];
				var hiddenTheadText = [];
				
				$scope.find('.wpr-table-th').each(function(index){
					if(index >= tableColumns) {
						$(this).css('display', 'none');
						hiddenTheadText.push($(this).text());
					}
				});

				beforeFilter.each(function(index){
					$(this).find('.wpr-table-td').each(function(index) {
						if(index >= tableColumns) {
							$(this).css('display', 'none');
						}

						if(index == 0) {
							$(this).addClass('wpr-table-tr-before');
						}
					})
				});

				$(document).on('click', '.wpr-table-tr-before', function() {

						$scope.find('.wpr-table-inner-container').attr('data-row-pagination');
						var colspanNum = 'yes' === $scope.find('.wpr-table-inner-container').attr('data-row-pagination') ? tableColumns + 1 : tableColumns;

						thisItemText = [];
						var tdTextCount = 0;

						$(this).toggleClass('wpr-table-tr-before-remove');

						$(this).closest('.wpr-table-row').find('.wpr-table-td').each(function(index) {
							if ( index >= tableColumns ) {
								thisItemText.push(`<li class="wpr-table-hidden-td" style="font-weight: bold; border-top: 1px solid black; border-bottom: 1px solid black;">${hiddenTheadText[tdTextCount]}</li><li class="wpr-table-hidden-td" style="font-weight: light;">${$(this).text() === '' ? 'empty' : $(this).text()}</li>`);
								tdTextCount++;
							} 
						});

						if( !$(this).closest('.wpr-table-row').next('.wpr-table-appended-tr').length ) {
							
							$(this).closest('.wpr-table-row').after('<tr class="wpr-table-appended-tr" style="align-items: center; width: 100%; text-align: center;"><td class="wpr-table-appended-td" colspan="'+ colspanNum +'" style="width: 100%; text-align: center; margin: auto;"><ul class="appended-ul">'+ thisItemText.toString().replace(/,/g, '') +'</ul></td></tr>');
			...
			$scope.find("#wpr-table-live-search").keyup(function () {
				...
				var afterFilter = initialRows.each(function (index) {
			...
			if ( $scope.find('.wpr-table-inner-container').attr('data-row-pagination') === 'yes' ) {
				$scope.find('.wpr-table-head-row').prepend('<th class="wpr-table-th-pag" style="vertical-align: middle;">' + 0 + '</th>')
			...
			function adjustPaginationList() {
				...
				paginationListItems.each(function(index) {
					if ( $(this).hasClass('wpr-active-pagination-item')) {
						paginationIndex = index;
					}

					if (index == 0 || index == paginationListItems.length - 1 || index <= paginationIndex + 1 && index >= paginationIndex - 1) {
			...
			if ( $scope.find('.wpr-table-export-button-cont').length ) {
				...
				exportBtn.each(function() {
					if ( $(this).hasClass('wpr-xls')) {
						...
						$(this).on('click', function() {
							htmlToCSV('why-this-arg?', "placeholder.csv", $scope.find('.wpr-advanced-table'));
						});
					}
				});
			}
			...
			function downloadCSVFile(csv, filename) {
				...
				download_link.click();
			} // Advanced Table CSV export

		}, // End widgetAdvancedTable
