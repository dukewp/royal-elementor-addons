	...
	var WprElements = {
		...
		widgetSection: function( $scope ) {
			if( $scope.attr('data-wpr-particles') || $scope.find('.wpr-particle-wrapper').attr('data-wpr-particles-editor') ) {
				WprElements.particlesEffect($scope);
            } 
			if ( $scope.hasClass('jarallax') || $scope.hasClass('jarallax-yes') ) {
				WprElements.jarallaxEffect($scope);
			}

			if($scope.hasClass('parallax-yes')) {
				parallaxLauncher();
			}

			function parallaxLauncher() {
				if( $scope.hasClass('parallax-yes') ) {
					var scene = document.getElementsByClassName('scene');

					var parallaxInstance = Array.from(scene).map(item => {
						console.log(item.getAttribute('scalar-x'));
						return new Parallax(item, {
							invertY: true,
							invertX: true,
							scalarX: item.getAttribute('scalar-x'),
							scalarY: item.getAttribute('scalar-y'),
							hoverOnly: true,
							pointerEvents: true
						});
					});
				...
				if ( !WprElements.editorCheck() ) {						
					...
					document.querySelectorAll('.scene').forEach((element, index) => {
		...
		particlesEffect: function( $scope ) {
			...
			if ( 'section' === elementType && undefined !== particlesJSON ) {
				...
				if ( ! WprElements.editorCheck() ) {
					...
					particlesJS('wpr-particle-'+ sectionID, $scope.attr('particle-source') == 'wpr_particle_json_custom' ? JSON.parse(particlesJSON) : WprElements.modifyJSON($scope, particlesJSON));
				// Editor
				} else {
					if ( $scope.hasClass('wpr-particle-yes') ) {
						particlesJS( 'wpr-particle-'+ sectionID, $scope.find('.wpr-particle-wrapper').attr('particle-source') == 'wpr_particle_json_custom' ? JSON.parse(particlesJSON) : WprElements.modifyJSON($scope, particlesJSON));
		...
		modifyJSON: function($scope, json) {
		...
		}, // end of functions for widget section
