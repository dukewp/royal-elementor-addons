( function( $, elementor ) {
	...
	var WprElements = {
        ...
        widgetImageAccordion: function($scope) {
			...
			lightboxPopup( lightboxSettings );

			// Lightbox Popup
			function lightboxPopup( settings ) {
				if ( -1 === $scope.find( '.wpr-img-accordion-item-lightbox' ).length ) {
					return;
				}
				...
				lightbox.each(function() {
					var source = $(this).find('.inner-block > span').attr( 'data-src' );
						...
						MediaWrap.find( '.wpr-accordion-image-wrap' ).attr( 'data-src', source );
				
				});

				// Init Lightbox
				MediaWrap.lightGallery( settings );
