		...
		widgetSection: function( $scope ) {
			...
			if( $scope.hasClass('wpr-sticky-section-yes') ) {
				var positionType = !WprElements.editorCheck() ? $scope.attr('data-wpr-position-type') : $scope.find('.wpr-sticky-section-yes-editor').attr('data-wpr-position-type'),
					elementOffset = !WprElements.editorCheck() ? $scope.attr('data-wpr-position-offset') : $scope.find('.wpr-sticky-section-yes-editor').attr('data-wpr-position-offset'),
					...
					customBpoints = !WprElements.editorCheck() ? $scope.attr('data-wpr-custom-breakpoints') : $scope.find('.wpr-sticky-section-yes-editor').attr('data-wpr-custom-breakpoints'),
					stickySectionExists = $scope.hasClass('wpr-sticky-section-yes') || $scope.find('.wpr-sticky-section-yes-editor') ? true : false,
					sectionClassList = $scope.attr('class'),
					positionStyle;
				...
				if( WprElements.editorCheck() && availableDevices ) {
					...
					$scope.attr('data-wpr-custom-breakpoints', customBpoints);
					availableDevices = $scope.attr('data-wpr-sticky-devices');
				}

				changePositionType(customBpoints);
				...
				if ( $('#wpadminbar').length && ('sticky' == $scope.css('position') || 'fixed' == $scope.css('position')) ) {
					var elementOffset = $scope.css('top').slice(0, $scope.css('top').length - 2),
						additionalOffset = $('#wpadminbar').css('height').slice(0, $('#wpadminbar').css('height').length - 2);
					positionType = positionType;
					elementOffset += additionalOffset;		
					$scope.css({ 'position': positionType,'top': elementOffset + 'px', 'bottom': 'auto', 'width': '100%' });
				} else {
					elementOffset
				}
			} else {	
			...
			function changeElementOffset() {
				for ( var i=0; i < sectionClassList.length - 1; i++ ) {

					if ( -1 !== sectionClassList[i].search(/offset-mobile\d/) ) {
						offsetMobile = +sectionClassList[i].replace('wpr-offset-mobile', '');
						console.log(offsetMobile);
					}
					
					if ( -1 !== sectionClassList[i].search(/offset-mobile_extra\d/) ) {
						offsetMobileExtra = +sectionClassList[i].replace('wpr-offset-mobile_extra', '');
						console.log(offsetMobileExtra);
					}
					
					if ( -1 !== sectionClassList[i].search(/offset-tablet\d/) ) {
						offsetTablet = +sectionClassList[i].replace('wpr-offset-tablet', '');
						console.log(offsetTablet);
					}
					
					if ( -1 !== sectionClassList[i].search(/offset-tablet_extra\d/) ) {
						offsetTabletExtra = +sectionClassList[i].replace('wpr-offset-tablet_extra', '');
						console.log(offsetTabletExtra);
					}
					
					if ( -1 !== sectionClassList[i].search(/offset-widescreen\d/) ) {
						offsetWideScreen = +sectionClassList[i].replace('wpr-offset-widescreen', '');
						console.log(offsetWideScreen);
					}
					
					if ( -1 !== sectionClassList[i].search(/offset-laptop\d/) ) {
						offsetLaptop = +sectionClassList[i].replace('wpr-offset-laptop', '');
						console.log(offsetLaptop);
					}

				}
			}

			function changePositionType(customBpoints) {
				changeElementOffset();
				if( !$scope.hasClass('wpr-sticky-section-yes') || !$scope.find('.wpr-sticky-section-yes-editor') ) {
					positionStyle = 'relative';
					return;
				}
				if ( 'inactive' === customBpoints || false === customBpoints ) {	

					if ( viewportWidth >= 1216 && availableDevices.indexOf('desktop_sticky') == -1 ) {
						positionStyle = 'relative';
					} else if ( (768 <= viewportWidth && 1216 > viewportWidth) && availableDevices.indexOf('tablet_sticky') === -1 ) {
						positionStyle = 'relative';
					} else if ( (360 <= viewportWidth && 767 > viewportWidth) && availableDevices.indexOf('mobile_sticky') === -1 ) {
						positionStyle = 'relative';
					} else {
						positionStyle = positionType;
					}

				} else if ( 'active' === customBpoints || 'default' === customBpoints) {

					console.log(positionType);

					if ( viewportWidth >= 2400 && availableDevices.indexOf('widescreen_sticky') === -1 ) {	
						positionStyle = activeDevices.indexOf('widescreen_sticky') !== -1 ? 'relative' : positionType;
						var widescreenSticky = positionStyle;
					} else if ( viewportWidth >= 2400 && availableDevices.indexOf('widescreen_sticky') !== -1 ) {
						positionStyle = positionType;
					}

					if ( (1216 <= viewportWidth && 2400 > viewportWidth) && availableDevices.indexOf('desktop_sticky') === -1 ) { //!stickySectionExists 
						positionStyle = activeDevices.indexOf('desktop_sticky') !== -1 ? 'relative' : widescreenSticky;
						var desktopSticky = positionStyle;
					} else if ( (1216 <= viewportWidth && 2400 > viewportWidth) && availableDevices.indexOf('desktop_sticky') !== -1 ) {
						positionStyle = positionType;
					}

					if ( (1216 > viewportWidth) && availableDevices.indexOf('laptop_sticky') === -1 ) { //1201 <= viewportWidth && 
						positionStyle = activeDevices.indexOf('laptop_sticky') !== -1 ? 'relative' : desktopSticky;
						var laptopSticky = positionStyle;
					} else if ( (1216 > viewportWidth) && availableDevices.indexOf('laptop_sticky') !== -1 ) {
						positionStyle = positionType;
					}

					if ( (1025 <= viewportWidth && 1201 > viewportWidth) && availableDevices.indexOf('tablet_extra_sticky') === -1 ) {
						positionStyle = activeDevices.indexOf('tablet_extra_sticky') !== -1 ? 'relative' : laptopSticky;
						var tabletExtraSticky = positionStyle;
					} else if ( (1025 <= viewportWidth && 1201 > viewportWidth) && availableDevices.indexOf('tablet_extra_sticky') !== -1 ) {
						positionStyle = positionType;
						console.log(positionStyle);
					}

					if ( (881 <= viewportWidth && 1025 > viewportWidth) && availableDevices.indexOf('tablet_sticky') === -1 ) {
						positionStyle = activeDevices.indexOf('tablet_sticky') !== -1 ? 'relative' : tabletExtraSticky;
						var tabletSticky = positionStyle;
					} else if ( (881 <= viewportWidth && 1025 > viewportWidth) && availableDevices.indexOf('tablet_sticky') !== -1 ) {
						positionStyle = positionType;
					}

					if ( (768 <= viewportWidth && 881 > viewportWidth) && availableDevices.indexOf('mobile_extra_sticky') === -1 ) {
						positionStyle = activeDevices.indexOf('mobile_extra_sticky') !== -1 ? 'relative' : tabletSticky;
						var mobileExtraSticky = positionStyle;
					} else if ( (768 <= viewportWidth && 881 > viewportWidth) && availableDevices.indexOf('mobile_extra_sticky') !== -1 ) {						
						positionStyle = positionType;
					}
					
					if ( (360 <= viewportWidth && 768 > viewportWidth) && availableDevices.indexOf('mobile_sticky') === -1 ) {  
						positionStyle = activeDevices.indexOf('mobile_sticky') !== -1 ? 'relative' : mobileExtraSticky;
						var mobileSticky = positionStyle;
					} else if ( (360 <= viewportWidth && 768 > viewportWidth) && availableDevices.indexOf('mobile_sticky') !== -1 ) {
						positionStyle = positionType;
					}
				}
