	...
	var WprElements = {
		...
		widgetSection: function( $scope ) {
			...
			if( $scope.hasClass('wpr-sticky-section-yes') ) {
				...
				positionStyle, setOffsets = [], adminBarHeight, replaceOffset;
...
>>>>>>> Stashed changes
					...
					var checkDevices = [['mobile_sticky', 768], ['mobile_extra_sticky', 881], ['tablet_sticky', 1025], ['tablet_extra_sticky', 1201], ['laptop_sticky', 1216], ['widescreen_sticky', 4000], ['desktop_sticky', 2400]];
					var emptyVariables = [];
					...
					// 		if(sectionClassList.filter((item) => {return item.match(replaceOffset[index][0])}).length == 0) {
					// 			return offset = offset[1] + '0';
					// 		} else {
					// 			return offset = offset[1] + sectionClassList.filter((item) => {return item.match(replaceOffset[index][0])}).join().replace(offset[1], '');
					// 		}
					// 	});
					// };
					checkedDevices.forEach((device, index) => {
						if ( (device[1] > viewportWidth) && availableDevices.indexOf(device[0]) === -1 ) {
							...
							device.push(positionStyle);
							console.log(positionStyle, emptyVariables[index]);
				...
				function applyPosition() {
					if ( 'top'  ===  positionLocation ) {
						$scope.css({ 'position': positionStyle, 'top': $scope.css('top'), 'bottom': 'auto', 'width': '100%' });
					}
					else {
						$scope.css({ 'position': positionStyle, 'bottom': $scope.css('bottom'), 'top': 'auto', 'width': '100%' }) ;
					}
				}
			} else {	
				$scope.css({ 'position': 'relative', 'top': '0', 'bottom': 'auto'}) ;
