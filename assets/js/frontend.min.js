	...
	var WprElements = {
		...
		init: function() {
			...
			var widgets = {
				...
				'wpr-woo-category-grid.default' : WprElements.widgetGrid,
		...
		widgetGrid: function( $scope ) {
			...
			if ( iGrid.find( '.wpr-grid-item-add-to-cart' ).length ) {
				...
				$( 'body' ).on( 'added_to_cart', function(ev, fragments, hash, button) {
					...
					if ( 'yes' === button.data('atc-popup') ) {
						var popupItem = button.closest('.wpr-grid-item');
						var popupText = popupItem.find('.wpr-grid-item-title').text();
						var popupLink = popupItem.find('.wpr-grid-item-title a').attr('href');
						var popupImageSrc = popupItem.find('.wpr-grid-image-wrap').length ? popupItem.find('.wpr-grid-image-wrap').data('src') : '';
						var fadeOutIn = button.data('atc-fade-out-in');
						var animTime = button.data('atc-animation-time');
						var popupImage;
						...
						$(this).find('.wpr-grid').append(`
							<div class="wpr-added-to-cart-popup">
								${popupImage}
						...
						setTimeout(() => {
							console.log('item fadeout');
							$(this).find('.wpr-added-to-cart-popup').addClass('wpr-added-to-cart-popup-hide');
							setTimeout(() => {
			...
			// Init Lightbox
			if (!$scope.hasClass('elementor-widget-wpr-woo-category-grid')) {
				lightboxPopup( settings );
			}
		...
		widgetProductMiniCart: function($scope) {
				...
				if ( 0 !== $scope.hasClass('wpr-mini-cart-sidebar').length ) {
					...
					const ulScroll = new PerfectScrollbar($scope.find('.wpr-woocommerce-mini-cart')[0], {
