( function( $, elementor ) {
	...
	var WprElements = {
		...
		widgetPostsTimeline: function($scope) { // goback
			var iScrollTarget = $scope.find( '.wpr-timeline-centered' ),
				pagination = $scope.find( '.wpr-grid-pagination' ),
				threshold = 'load-more' === iScrollTarget.attr('data-pagination') ? false : 10,
			...
			function removeLeftAlignedClass() {
				...
				// console.log($('body').attr('data-elementor-device-mode'));
				// var maxBreakpoint = 'mobiel' === $('body').attr('data-elementor-device-mode') ? 881 : 768;
			...
			window.addEventListener('scroll',  function() {
				setTimeout(() => postsTimelineFill(lastIcon, firstIcon), 0);
			});
			...
			if ( $scope.find('.swiper-wrapper').length ) {
				...
				if ( $scope.find('.swiper-slide.swiper-slide-line-top.auto-height .wpr-story-info').length ) {
					// $scope.find('.swiper-slide.swiper-slide-line-top.auto-height .wpr-story-info').css('height', 'calc(100% - '+ $scope.find('.swiper-slide.auto-height .wpr-story-info').css('marginTop') +')');
				} else if ( $scope.find('.swiper-slide.swiper-slide-line-bottom.auto-height .wpr-story-info').length ) {
					// $scope.find('.swiper-slide.swiper-slide-line-bottom.auto-height .wpr-story-info').css('height', 'calc(100% - '+ $scope.find('.swiper-slide.auto-height .wpr-story-info').css('marginBottom') +')');  
				}
		  ...
		  function postsTimelineFill(lastIcon, firstIcon) {
			...
			AOS.init({
				...
				duration: aosDuration,
				once: aosOnce,
			}); //todo remove attributes in not needed

			if ( !$scope.find('.wpr-timeline-fill').length ) {
				return;
			}

			if ( $scope.find('.wpr-timeline-entry-inner:eq(0) .wpr-year-label').length > 0 ) {
				console.log('change occurs');
				firstIcon = $scope.find('.wpr-timeline-entry-inner:eq(0) .wpr-year-label').eq(0);
			}
			  ...
			  if( timelineFill.length ) {
				...
				if ( !((docScrollTop + clientHeight - (firstIcon.offset().top)) > lastIcon.offset().top - firstIcon.offset().top + parseInt(lastIcon.css('height').slice(0, -2))) ) {
				  ...
				  $('.wpr-main-line-icon.wpr-icon').each(function() {
		  ...
		  function adjustMiddleLineHeight(middleLine, timelineFill, lastIcon, firstIcon, element) {
				...
				middleLine.css('top', (firstIconOffset - element.offset().top) + 'px');
				// middleLine.css('height', (lastIconOffset - (lastIcon.css('height').slice(0, -2)/2 + (firstIconOffset - firstIcon.css('height').slice(0, -2)/2))) + 'px');
				middleLine.css('height', (lastIconOffset - (lastIcon.css('height').slice(0, -2)/2 + (firstIconOffset - firstIcon.css('height').slice(0, -2)))) + 'px');
				middleLine.css('maxHeight', firstIconOffset - lastIconOffset + 'px !important');
				timelineFill.css('top', (firstIconOffset - element.offset().top) + 'px');
		  }
