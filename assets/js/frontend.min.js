	...
	var WprElements = {
		...
		widgetAdvancedTable: function($scope) {
			...
			if ($scope.find('.wpr-table-th').length > 8) {
				...
				beforeFilter.find('.wpr-table-tr-before').each(function(index) {
					...
					$(this).on('click', function(index) {
						console.log('works');
							...
							$(this).closest('.wpr-table-row').find('.wpr-table-td').each(function(index) {
								if ( index >= 8 ) {
									thisItemText.push(`<li class="wpr-table-hidden-td">${hiddenTheadText[tdTextCount]}</li><li class="wpr-table-hidden-td">${$(this).text()}</li>`);
									tdTextCount++;
								} 
							});

							if( !$(this).closest('.wpr-table-row').next('.wpr-table-appended-tr').length ) {
								$(this).closest('.wpr-table-row').after('<tr class="wpr-table-appended-tr"><td class="wpr-table-appended-td" style="width: 100%; text-align: center;"><ul class="appended-ul">'+ thisItemText.toString().replace(/,/g, '') +'</ul></td></tr>');
			...
			// if ( $scope.find('.wpr-table-from-csv-file').length ) {

			// 	console.log($scope.find('.wpr-table-from-csv-file').attr('data-wpr-csv-file'));

			// 	var csvtotable = new CsvToTable({
			// 		csvFile: $scope.find('.wpr-table-from-csv-file').attr('data-wpr-csv-file')
			// 	});

			// 	csvtotable.run($scope);

			// 	var tableFromCSV = $(document).find('.wpr-append-to-scope');
			...
			if ( $scope.find('.wpr-table-export-button-cont').length ) {
				...
				exportBtn.each(function() {
					if ( $(this).hasClass('wpr-xls')) {
						$(this).on('click', function() {
							// fnExcelReport($scope.find('.wpr-advanced-table'));
							// exportToExcel($scope);        
			...
			function fnExcelReport(html) {
				var tab_text="<table border='2px'><tr bgcolor='#87AFC6'>";
				var textRange; var j=0;
				var tab = html; // id of table
		
				for(j = 0 ; j < tab.find('.wpr-table-row').length ; j++) 
				{     
					tab_text=tab_text+tab.find('.wpr-table-row')[j].innerHTML+"</tr>";
				...
				return (sa);
			} // Advanced Table XLS export

			function exportToExcel($scope){
				var htmls = $scope.find('.wpr-advanced-table').prop('outerHTML');
