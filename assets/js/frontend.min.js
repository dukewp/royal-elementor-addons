	...
	var WprElements = {
		...
		init: function() {
			...
			var widgets = {
				...
				'wpr-product-media-old.default' : WprElements.widgetProductMediaOld,
				...
				'wpr-my-account.default' : WprElements.widgetPageMyAccount,
				...
				$( 'body' ).on( 'added_to_cart', function(ev, fragments, hash, button) {
					...
					if ( 'sidebar' === button.data('atc-popup') ) {
						...
						var popupItem = button.closest('.wpr-grid-item'),
							...
							popupLink = popupItem.find('.wpr-grid-item-title a').attr('href'),
						...
						if ( '' !== popupImageSrc ) {
							popupImage = `<div class="wpr-added-tc-popup-img">
						...
						$(this).find('.wpr-grid').append(`
							<div class="wpr-added-to-cart-popup ${animationClass}">
								${popupImage}
								<div class="wpr-added-tc-title">
									<p>${popupText} was added to cart</p>
									<p><a href=${popupLink}>View Cart</a></p>
								</div>
							</div>
						`);
		...
		widgetProductMediaOld: function( $scope ) {
			...
			if ( '[]' !== gallerySettings && gallery.length ) {
				...
				if ( 'yes' === gallerySettings.thumbnail_nav ) {
					...
					navigation.find( '.slick-slide' ).on( 'click', function () {
		...
		widgetProductMedia: function( $scope ) {
			...
			var swiperSlider = $scope.find('.woocommerce-product-gallery');
			var swiperNavigation = '<div class="wpr-thumb-slider-wrap"><button class="wpr-thumbnail-slider-prev-arrow">prev</button><button class="wpr-thumbnail-slider-next-arrow">next</button></div>';
			swiperSlider.prepend(swiperNavigation);
			var posx = 0;
			...
			$scope.find('.wpr-thumbnail-slider-next-arrow').on('click', function() {
					...
					if ( posx < $scope.find('.flex-control-nav li').length - 1 ) {
						posx++;
						console.log(posx);
						$scope.find('.flex-control-nav').css('transform', `translateX(${parseInt(-posx) * parseInt($scope.find('.flex-control-nav li').css('width').slice(0, -2))}px)`);
						if ( posx >= 1 ) {
			...
			// 	var swiperNavigation = '<div class="swiper-button-prev wpr-thumbnail-slider-prev-arrow"></div><div class="swiper-button-next wpr-thumbnail-slider-next-arrow"></div>';
			// 	swiperSlider.append(swiperNavigation);
			// 	swiperSlider.append('<div class="swiper-pagination"></div>');
		...
		widgetProductMiniCart: function($scope) {
				...
				if ( 0 !== $scope.hasClass('wpr-mini-cart-sidebar').length ) {
					...
					if ( $scope.find('.wpr-woo-mini-cart').length ) {
						console.log('perfectscrollbar');
						const ulScroll = new PerfectScrollbar($scope.find('.wpr-woo-mini-cart')[0], {
