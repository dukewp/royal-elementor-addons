( function( $, elementor ) {
	...
	var WprElements = {
        ...
        widgetVideoPlaylist: function($scope) {
            var activeItemSrc = $scope.find('.wpr-video-active').find('a').attr('href');
            var activeItemSrcEmbed = activeItemSrc.replace('watch', 'embed');
            var activeItem = `<iframe width="420" height="345" src="${activeItemSrcEmbed}"></iframe>`;
            $scope.find('.wpr-video-playlist-item-active').html(activeItem);
            $scope.find('.wpr-video-playlist-item').click(function(){
                $scope.find('.wpr-video-playlist-item').removeClass('wpr-video-active');
                $(this).addClass('wpr-video-active'); 
                activeItemSrc = $scope.find('.wpr-video-active').find('a').attr('href');
                activeItemSrcEmbed = activeItemSrc.replace('watch', 'embed');
                $scope.find('.wpr-video-playlist-item-active').find('iframe').attr('src', activeItemSrcEmbed);
            });
        }, // end widgetFeatureList
