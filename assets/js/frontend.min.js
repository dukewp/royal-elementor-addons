	...
	var WprElements = {
		...
		widgetGrid: function( $scope ) {
			...
			// Slider
				...
				var	sliderScrollDesktop = +(sliderClass.match(/wpr-grid-slides-to-scroll-\d/) ? sliderClass.match(/wpr-grid-slides-to-scroll-\d/).join().slice(-1) : 2),
					sliderScrollTablet = +(sliderClass.match(/scroll--tablet\d/) ? sliderClass.match(/scroll--tablet\d/).join().slice(-1) : 2),
					sliderScrollMobile = +(sliderClass.match(/scroll--mobile\d/) ? sliderClass.match(/scroll--mobile\d/).join().slice(-1) : 1);

				if ( 1 === +sliderColumnsDesktop ) {
					sliderScrollDesktop = 1;
				}

				if ( 1 === +sliderColumnsTablet ) {
					sliderScrollTablet = 1;
				}

				if ( 1 === +sliderColumnsMobile ) {
					sliderScrollMobile = 1;
				}
console.log(sliderScrollDesktop +' - '+ sliderColumnsTablet +' - '+ sliderColumnsMobile)
				iGrid.slick({
					...
					// slidesToScroll: sliderScrollDesktop,
					responsive: [
						{
							breakpoint: 10000,
							settings: {
							  slidesToShow: sliderClass.match(/columns--widescreen\d/) ? sliderClass.match(/columns--widescreen\d/).join().slice(-1) : sliderColumnsDesktop,
							  // slidesToScroll: +(sliderClass.match(/scroll--widescreen\d/) ? sliderClass.match(/scroll--widescreen\d/).join().slice(-1) : sliderScrollDesktop),
							}
						},
						{
							breakpoint: 2399,
						...
						},
						{
							breakpoint: 1221,
							settings: {
							  slidesToShow: sliderClass.match(/columns--laptop\d/) ? sliderClass.match(/columns--laptop\d/).join().slice(-1) : sliderColumnsDesktop,
							  // slidesToScroll: +(sliderClass.match(/scroll--laptop\d/) ? sliderClass.match(/scroll--laptop\d/).join().slice(-1) : sliderScrollDesktop),
							}
						},
						{
							breakpoint: 1200,
							settings: {
							  slidesToShow: sliderClass.match(/columns--tablet_extra\d/) ? sliderClass.match(/columns--tablet_extra\d/).join().slice(-1) : sliderColumnsTablet,
							  // slidesToScroll: +(sliderClass.match(/scroll--tablet_extra\d/) ? sliderClass.match(/scroll--tablet_extra\d/).join().slice(-1) : sliderScrollTablet),
							}
						},
						{
						  breakpoint: 1024,
						  settings: {
							...
							// slidesToScroll: sliderScrollTablet,
						  }
						},
						{
						  breakpoint: 880,
						  settings: {
							slidesToShow: sliderClass.match(/columns--mobile_extra\d/) ? sliderClass.match(/columns--mobile_extra\d/).join().slice(-1) : sliderColumnsTablet,
							// slidesToScroll: +(sliderClass.match(/scroll--mobile_extra\d/) ? sliderClass.match(/scroll--mobile_extra\d/).join().slice(-1) : sliderScrollTablet),
						  }
						},
						{
						  breakpoint: 768,
						  settings: {
							...
							// slidesToScroll: sliderScrollMobile,
						  }
						}
					],
				});
