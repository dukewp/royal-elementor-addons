		...
		widgetSection: function( $scope ) {
			...
			if( $scope.hasClass('wpr-sticky-section-yes') ) {
				var positionType = !WprElements.editorCheck() ? $scope.attr('data-wpr-position-type') : $scope.find('.wpr-sticky-section-yes-editor').attr('data-wpr-position-type'),
					...
					positionStyle, setOffsets, offsetMobile, offsetMobileExtra, offsetTablet, offsetTabletExtra, offsetWideScreen, offsetLaptop, offsetDesktop;
			...
			function changeElementOffset() {
				setOffsets = [offsetMobile, offsetMobileExtra, offsetTablet, offsetTabletExtra, offsetWideScreen, offsetLaptop, offsetDesktop];
				var deviceRegEx = [/offset--mobile\d/, /offset--mobile_extra\d/, /offset--tablet\d/, /offset--tablet_extra\d/, /offset--widescreen\d/, /offset--laptop\d/, /offset-\d/];

				var replaceOffset = ['wpr-offset-mobile', 'wpr-offset--mobile_extra', 'wpr-offset--tablet', 'wpr-offset--tablet_extra', 'wpr-offset--widescreen', 'wpr-offset--laptop', 'wpr-offset-'];
			...
			function changePositionType(customBpoints) {
				...
				if ( 'inactive' === customBpoints || false === customBpoints ) {	
					...
					checkDevices.forEach((device, index) => {
						if ( (breakPointSizes[index] > viewportWidth) && availableDevices.indexOf(device) === -1 ) { //!stickySectionExists 
							positionStyle = activeDevices.indexOf(device) !== -1 ? 'relative' : widescreenSticky;
							emptyVariables[index] = positionStyle;
						} else if ( (breakPointSizes[index] > viewportWidth) && availableDevices.indexOf(device) !== -1 ) {
							positionStyle = positionType;
							elementOffset = setOffsets[index]; // offsetDesktop
						}
					});
				}
