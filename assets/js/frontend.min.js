( function( $, elementor ) {
	...
	var WprElements = {
		...
		widgetAdvancedTable: function($scope) {
			...
			if ($scope.find('.wpr-table-th').length > 10) {
				...
				$scope.find('.wpr-table-th').each(function(index) {
					if (index >= 9) {
						hiddenTheadText.push(`<li>${$(this).text()}</li>`);
					}
				});

				console.log(hiddenTheadText);

				$scope.find('.wpr-table-tr-before').each(function(index) {

					$(this).closest('.wpr-table-row').find('.wpr-table-td').each(function(index) {
						if ( index >= 9 ) {
							thisItemText.push($(this).text());
						} 
					});

					$(this).on('click', function(index) {

							if( !$scope.find('.wpr-table-appended-tr').length ) {
								console.log(thisItemText[1]);
								console.log(hiddenTheadText[0]);
								$(this).addClass('wpr-table-tr-before-remove');
								$(this).closest('.wpr-table-row').after('<tr class="wpr-table-appended-tr"><td><ul class="appended-ul">'+ hiddenTheadText +'</ul></td></tr>');

								// $(this).closest('.wpr-table-row').find('.wpr-table-appended-tr td ul').append();

								
							} else {
								$(this).removeClass('wpr-table-tr-before-remove');
								$scope.find('.wpr-table-appended-tr').remove();
							}
