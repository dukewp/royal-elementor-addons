			...
			function changeElementOffset() {
				...
				for ( var i=0; i < sectionClassList.length - 1; i++ ) {
					if ( -1 !== sectionClassList[i].search(deviceRegEx[i]) ) {
						...
						console.log(setOffsets[i], sectionClassList[i], replaceOffset[i]);
					} else if ( -1 == sectionClassList[i].search(deviceRegEx[i]) ) {
						setOffsets[i] = 0;
						console.log(sectionClassList[i]);
					}
				}
			}

			function changePositionType(customBreakPoints) {
				if( !$scope.hasClass('wpr-sticky-section-yes') || !$scope.find('.wpr-sticky-section-yes-editor') ) {
					positionStyle = 'relative';
					return;
				}
				if ( 'inactive' === customBreakPoints || false === customBreakPoints ) {	
				...
				} else if ( 'active' === customBreakPoints || 'default' === customBreakPoints) {					
					changeElementOffset();
					...
					checkDevices.forEach((device, index) => {
						if ( (breakPointSizes[index] > viewportWidth) && availableDevices.indexOf(device) === -1 ) { //!stickySectionExists 
							...
							console.log(device, elementOffset);
						}
					});
				}
