	...
	var WprElements = {
		...
		init: function() {
			...
			var widgets = {
				...
				'wpr-product-menu-cart.default' : WprElements.widgetProductMenuCart,
			...
			function ajaxAddToCart(e) {
				...
				$.ajax( {
					...
					success: function ( response ) {
						...
						var currentCartCount = parseInt($('.wpr-menu-cart-icon-count').text());
						var updatedCartCount = parseInt($scope.find('.wpr-quantity-wrapper .qty').val());
						$('.wpr-menu-cart-icon-count').text(currentCartCount + updatedCartCount);
					},
				} );
			}
		}, // End of widgetProductAddToCart

		widgetProductMenuCart: function($scope) {
				var displayMiniCart = false;
				$scope.find('.wpr-menu-cart-toggle-btn').on('click', function(e) {
					e.preventDefault();
					displayMiniCart = !displayMiniCart;
					if(displayMiniCart == false) {
						$scope.find('.wpr-menu-cart').css({"display": "none"});
					} else {
						$scope.find('.wpr-menu-cart').css({"display": "block"});
					}
				});
		},
		...
		widgetPageMyAccount: function($scope) {
			console.log(WprElements.editorCheck());
			var targetTab;

			setTimeout(function(){
				targetTab = $scope.find('.woocommerce-MyAccount-navigation-link');
				console.log(targetTab);

				targetTab.click( 'a', function(item) {
					editorShowTab();
				});
			}, 3000);
